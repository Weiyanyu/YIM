...
public class PacketCodec {
    ...
    //packet类型的Map
    private static final Map<Byte, Class<? extends AbstractPacket>> packetTypeMaps;
     ...
     * 编码
     * @param buf 外部传进来的buf
     * @param abstractPacket 数据对象
     * @return 填满的buf
     */
    public ByteBuf encode(ByteBuf buf, AbstractPacket abstractPacket) {
        //填充魔数
        buf.writeInt(MAGIC_NUMBER);
        //版本号
        buf.writeByte(abstractPacket.getVersion());
        //序列化算法
        buf.writeByte(Serializer.DEFAULT_SERIALIZER.getSerializerAlgorithm());
        //指令
        buf.writeByte(abstractPacket.getCommand());
        //数据长度
        byte[] data = Serializer.DEFAULT_SERIALIZER.serialize(abstractPacket);
        int len = data.length;
        buf.writeInt(len);
        //数据
        buf.writeBytes(data);
